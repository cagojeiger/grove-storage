...
외부 S3 backend와 운영 데이터 이관은 미검증이다.
...
| 테스트 | 단위·실제 PG·SDK HTTP의 층이 생김 | 실제 외부 S3 backend·응답 유실·프로세스 중단 검증 |
...
다음 우선순위는 crate 수를 늘리는 것보다 외부 S3 backend에서 같은 계약을 재검증하는 것이다.
